...
#' Note the edge associated with anagenic species is the edge on which the speciation event occurred.
#'
#' @examples
#'
#' @export
add.anagenic.species<-function(tree, species, lambda.a){
  ...
  # if true, only the speciation edge associated with anagenic species is returned
  speciation.edge.only = FALSE
  ...
  if(lambda.a > 0){
    ...
    # this is set to older than the greatest edge label id to avoid confusion with species labels
