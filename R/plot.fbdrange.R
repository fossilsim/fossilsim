...
plot.fbdrange <- function(x, smart.labels = FALSE, ...) {
  for(pname in c("ggtree", "ggfun", "ggplot2", "tidytree")) {
    if (!requireNamespace(pname, quietly = TRUE)) {
      stop(paste("Package", pname, "is needed for plotting range trees. Please install it."), call. = FALSE)
    }
  }

  p1 <- ggplot2::ggplot(x, ggplot2::aes(x, y, color = range)) + ggtree::geom_tree(linewidth=2) +
    ggplot2::geom_point(ggplot2::aes(color = range), size = 1.3)
  ...
  if(smart.labels) {
    ...
    p1 <- ggfun::`%<+%`(p1, labeldf)
    p1 <- p1 + do.call(ggrepel::geom_label_repel, c(list(mapping = ggplot2::aes(label = new_labels, alpha = 0.7, fontface = 4)), label_args))
  }
