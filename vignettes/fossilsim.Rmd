...
This vignette provides detailed examples of available functions for simulating data, plotting the package output and interacting with other R packages and programs.
...
The object also contains a logical variable `from.taxonomy` indicating whether the fossil record was simulating using a `taxonomy` object, in which case `from.taxonomy = TRUE`, or from a `phylo` object in which case `from.taxonomy = FALSE` (the default).

## Simulating taxonomy

Species evolution or taxonomy can be simulated under a mixed model of speciation, which incorporates three modes of speciation: budding (or asymmetric), bifurcating (or symmetric) and anagenetic speciation.
\todo add citation Stadler 2018
Under this model, $\beta$ is the probability that a branching event corresponds to a bifurcation event, while $1 - \beta$ is the probability that the event corresponds to a budding event.
If $\beta = 0$ all speciation occurs via budding. 
If $\beta = 1$ all speciation occurs via bifurcation - this is equivalent to assuming all edges in a phylogenetic tree correspond to a unique species. 
