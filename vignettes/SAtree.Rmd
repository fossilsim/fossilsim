...
t = ape::rtree(6)
tax = sim.taxonomy(t)
f = sim.fossils.poisson(rate = 2, taxonomy = tax)

SAt = SAtree.from.fossils(t,f)
plot(SAt)
```

`SAtree` objects are also `phylo` objects and can still be used with all the functions compatible with that format.
```{r}
ape::plot.phylo(SAt)
```
...
`sampled.tree.from.combined` removes all unsampled lineages from the tree, and can optionally apply a sampling probability to extant samples, or keep only a specified list of extant samples. NB: extinct tips will be considered unsampled by default unless they are sampled ancestors.
