...
Many phylogenetic inference software packages, such as [BEAST2](http://www.beast2.org/) and [RevBayes](https://revbayes.github.io), can handle trees containing fossil samples. In the zero-edge format, tip samples are represented normally in the tree, but sampled ancestors, i.e fossil samples which have sampled descendants, are represented as tips at the end of edges with length 0.
...
`prune.SAtree.to.ranges` will remove all intermediate fossil samples and keep only the first and last occurrences of any species.

```{r}
SAt_pruned = prune.SAtree.to.ranges(tree = SAt$tree)$tree
plot(SAt_pruned)
```
...
```{r}
SAt_sampled = sampled.tree.from.combined(tree = SAt$tree)$tree
plot(SAt_sampled)
```

These two functions can be applied to any object of type `phylo`, but are designed to work on trees which include fossil samples. Note that all functions which operate on `SAtree` objects also accept an optional `taxonomy` object as argument (and some also accept a `fossils` object). This allows the taxonomy and fossil dataframes to be updated alongside the tree, for instance when edges are added or removed, so that all objects remain consistent with each other.
