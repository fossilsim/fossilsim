...
The function output can be used to represent changes in relative water depth, where the number of cycles may be equivalent to the number of trangression/regression events.
...
The function output is a vector of simulated trait values that can be used to specify the `rate` pararmeter in `sim.fossils.poisson` or the `PD`, `DT` and `PA` parameters in `sim.fossils.environment`.

Trait values can be simualted for a given `phylo` or `taxonomy` object. 
If the function is provided with a `phylo` object, trait values are simulated assumming bifurcating speciation and simulated trait values are output for each edge in the order in which they appear in the `phylo` object dataframe.
